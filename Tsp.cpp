/* Tsp.cpp
* Have added omp for the for loops in select() and populate()
...
// Calculate distance between citites
extern void computeDistanceBetweenCities(int coordinates[CITIES][2]);

// need to implement for your program 1
extern void evaluate(Trip trip[CHROMOSOMES], int coordinates[CITIES][2]);
extern void crossover(Trip parents[TOP_X], Trip offsprings[TOP_X], 
...
int main(int argc, char* argv[]) {
    ...
    float distances[CITIES][CITIES] = { 0 };
    float distancesOrigin[CITIES] = { 0 };
    int nThreads = 1;
    ...
    computeDistanceBetweenCities(coordinates);
...
/*
* Initializes trip[CHROMOSOMES] with chromosome.txt and 
* coordiantes[CITIES][2] with cities.txt
*
* @param trip[CHROMOSOMES]:      50000 different trips
* @param coordinates[CITIES][2]: (x, y) coordinates of 36 different 
...
void select(Trip trip[CHROMOSOMES], Trip parents[TOP_X]) {
    ...
    #pragma omp parallel for default(none) firstprivate(trip, parents)  // parallelization
...
// just copy TOP_X offsprings to the bottom TOP_X trips.
    ...
    for (int i = 0; i < TOP_X; i++)
        strncpy(trip[CHROMOSOMES - TOP_X + i].itinerary, 
            offsprings[i].itinerary, CITIES + 1);
